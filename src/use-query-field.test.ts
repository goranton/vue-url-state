...
describe('useQueryField', () => {
  ...
  it('calls onError when query.patch rejects', async () => {
    const stateRef = ref<SchemaState>({
      search: undefined,
      page: 1,
      status: undefined,
    ...
    const searchField = useQueryField(query, 'search', { onError });
    searchField.value = 'anton';
  ...
  it('swallows rejected patch when onError is not provided', async () => {
    const stateRef = ref<SchemaState>({
      search: undefined,
      page: 1,
      status: undefined,
